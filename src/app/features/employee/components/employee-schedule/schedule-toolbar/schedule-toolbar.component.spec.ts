...
class TestHostComponent {
    mockTableComponent = new MockScheduleTable() as unknown as ScheduleTableComponent;
}
...
describe('ScheduleToolbarComponent', () => {
    ...
    beforeEach(async () => {
        ...
        const debugEl = fixture.debugElement.query(By.directive(ScheduleToolbarComponent));
    ...
    it('should call openScheduleDialog on addNew', () => {
        const addButton = fixture.debugElement.query(By.css('p-button[label="New"]'));
        if (addButton) {
            const component = addButton.componentInstance;
            component.onClick.emit();

            expect(scheduleToolbarComponent.scheduleStore.openScheduleDialog).toHaveBeenCalled();
        }
    });
