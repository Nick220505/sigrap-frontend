...
describe('CustomerDialogComponent', () => {
    ...
    beforeEach(async () => {
        ...
        customerStore = {
            ...
            closeCustomerDialog: vi.fn().mockName("CustomerStore.closeCustomerDialog"),
    ...
    it('should show "New Customer" header when no customer is selected', () => {
        ...
        (customerStore.selectedCustomer as WritableSignal<CustomerInfo | null>).set(null);
        fixture.detectChanges();
    ...
    it('should show "Edit Customer" header when a customer is selected', () => {
        (customerStore.dialogVisible as WritableSignal<boolean>).set(true);
        (customerStore.selectedCustomer as WritableSignal<CustomerInfo | null>).set(mockCustomer);
        fixture.detectChanges();
    ...
    it('should patch form with customer data when editing', () => {
        ...
        expect(component.customerForm.get('fullName')?.value).toBe(mockCustomer.fullName);
        expect(component.customerForm.get('documentId')?.value).toBe(mockCustomer.documentId);
        expect(component.customerForm.get('email')?.value).toBe(mockCustomer.email);
        expect(component.customerForm.get('phoneNumber')?.value).toBe(mockCustomer.phoneNumber);
        expect(component.customerForm.get('address')?.value).toBe(mockCustomer.address);
    });
