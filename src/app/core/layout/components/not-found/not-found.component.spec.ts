...
describe('NotFoundComponent', () => {
    ...
    describe('UI elements', () => {
        it('should display 404 error code', () => {
            const errorCode = fixture.debugElement.query(By.css('.text-primary.font-bold.text-3xl'));
        ...
        it('should display error description message', () => {
            const description = fixture.debugElement.query(By.css('.text-surface-600.dark\\:text-surface-200.mb-8'));
            expect(description).toBeTruthy();
            expect(description.nativeElement.textContent.trim()).toContain('The requested resource does not exist');
        });
    ...
    describe('Navigation links', () => {
        it('should have link to General Inventory', () => {
            const inventoryLink = fixture.debugElement.query(By.css('a[routerLink="/inventory/products"]'));
            expect(inventoryLink).toBeTruthy();

            const linkTitle = inventoryLink.query(By.css('.text-surface-900.dark\\:text-surface-0'));
            expect(linkTitle.nativeElement.textContent.trim()).toBe('General Inventory');
        ...
        it('should have link to Entry Management', () => {
            const entradasLink = fixture.debugElement.queryAll(By.css('a[routerLink="/"]'))[0];
            expect(entradasLink).toBeTruthy();

            const linkTitle = entradasLink.query(By.css('.text-surface-900.dark\\:text-surface-0'));
            expect(linkTitle.nativeElement.textContent.trim()).toBe('Entry Management');
        ...
        it('should have link to Exit Management', () => {
            const salidasLink = fixture.debugElement.queryAll(By.css('a[routerLink="/"]'))[1];
            expect(salidasLink).toBeTruthy();

            const linkTitle = salidasLink.query(By.css('.text-surface-900.dark\\:text-surface-0'));
            expect(linkTitle.nativeElement.textContent.trim()).toBe('Exit Management');
        ...
        it('should have button to go to Panel Principal', () => {
            const mainPanelButton = fixture.debugElement.query(By.css('p-button[routerLink="/"]'));
