...
describe('ConfiguratorComponent', () => {
    ...
    describe('getPresetExt method', () => {
        it('should return noir preset config when color.name is noir', () => {
            ...
            expect(noirResult.semantic.colorScheme.light.primary.color).toBe('{primary.950}');
            expect(noirResult.semantic.colorScheme.dark.highlight.color).toBe('{primary.950}');
        });
        ...
        it('should return Nora preset config when preset is Nora', () => {
            ...
            expect(noraResult.semantic.colorScheme.light.primary.color).toBe('{primary.600}');
            expect(noraResult.semantic.colorScheme.dark.primary.color).toBe('{primary.500}');
        });
        ...
        it('should return default preset config for other cases', () => {
            ...
            expect(defaultResult.semantic.colorScheme.light.primary.color).toBe('{primary.500}');
            expect(defaultResult.semantic.colorScheme.dark.highlight.background).toContain('color-mix');
        });
    });
    ...
    describe('onPresetChange method', () => {
        it('should update layout config when preset changes', () => {
            ...
            const updateCall = calls[calls.length - 1][0] as (config: { preset: string }) => { preset: string };
        ...
        it('should not update config when preset value is not valid', () => {
            ...
            expect(layoutConfigUpdateSpy).toHaveBeenCalled();
            const updateCall = (layoutConfigUpdateSpy as Mock).mock.calls[0][0] as (config: { preset: string }) => { preset: string };
    ...
    describe('UI interaction', () => {
        it('should trigger updateColors when primary color button is clicked', () => {
            ...
            const primaryColorButtons = fixture.debugElement.queryAll(By.css('button[title]'));
    ...
    describe('Menu Mode changes', () => {
        it('should update layout config when menu mode is changed', () => {
            ...
            expect(layoutConfigUpdateSpy).toHaveBeenCalled();
            const updateCall = (layoutConfigUpdateSpy as Mock).mock.calls[0][0] as (config: { menuMode: string }) => { menuMode: string };
